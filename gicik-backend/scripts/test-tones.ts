// Tone A/B test — aynı senaryo, archetype sabit (observer = nötr), 5 tone değişken.
// Her ton için 3 reply de aynı tonda üretilir → tone karakteri net görünür.
//
// Usage:
//   cd gicik-backend
...
// Sabit archetype — en nötr olanı (observer). chaos_agent gibi dominant
// archetype seçersek tone'u ezebilir; observer iken tone karakteri net çıkar.
const FIXED_ARCHETYPE = "observer";
...
// ─── Aynı senaryo: tinder, 3 gün sessizlik sonrası sitem ───
const PARSE_RESULT = {
  participants: [
    { role: "user", name: null },
...
function buildUserPrompt(tone: string): string {
  ...
  "replies": [
    {"index": 0, "tone": "${tone}", "text": "string (max 280 char)"},
...
// ─── main ───
const [L0, L1, L2, L4Raw, archetypePrompt] = await Promise.all([
  loadPrompt("L0"),
  loadPrompt("L1", { mode: "cevap" }),
  loadPrompt("L2"),
  loadPrompt("L4"),
  loadPrompt("archetype", { archetype: FIXED_ARCHETYPE }),
]);

console.log(`\n═══════════════════════════════════════════════════════════════════`);
console.log(`SENARYO: tinder, 3 gün sessizlik, karşı taraf "neden yazmıyorsun ya"`);
console.log(`ARCHETYPE: 🪨 observer (sabit, en nötr)  |  TONE: değişken`);
console.log(`═══════════════════════════════════════════════════════════════════\n`);

for (const tone of TONES) {
  const tonePrompt = await loadPrompt("tone", { tone });
  const tonesBlock = [0, 1, 2].map(i => `--- TON ${i + 1} (${tone}) ---\n${tonePrompt}`).join("\n\n");
  const L4Filled = fillL4(L4Raw, FIXED_ARCHETYPE, tone);
  ...
  console.log(`──────────────────────────────────────`);
  console.log(`${TONE_LABELS[tone]}  (${tone})`);
  console.log(`──────────────────────────────────────`);
  try {
